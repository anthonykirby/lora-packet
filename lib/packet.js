...
//        var type = p.MHDR._getMType;
    ...
    function _isPlausible(userFields) {
        if (!_isDefined(userFields)) {
            return false;
        }
        else if (
            _isDefined(userFields.payload)
            && _isDefined(userFields.DevAddr)
    ...
    function _initialiseFromFields(userFields) {
        ...
        // required fields
        if (_isDefined(userFields.DevAddr)
            && userFields.DevAddr instanceof Buffer
            && userFields.DevAddr.length==4) {
            p.DevAddr = new Buffer(userFields.DevAddr);
        } else { return null }

        // required fields:
        if (_isDefined(userFields.payload)) {
            // construct from string
            if (typeof userFields.payload === 'string' || userFields.payload instanceof String) {
                // TODO
        ...
        if (_isDefined(userFields.MHDR)) {
            if (typeof(userFields.MHDR)=='number') {
                p.MHDR = new Buffer(1);
                p.MHDR.writeUInt8(userFields.MHDR << 5, 0);
            } else {
                var mhdr_idx = MTYPE_DESCRIPTIONS.indexOf(p.MHDR);
                if (mhdr_idx >= 0) {
                    p.MHDR = new Buffer(1);
        ...
        // defaults
        if (!_isDefined(p.MHDR)) {
            p.MHDR = new Buffer(1);
            p.MHDR.writeUInt8(MTYPE_UNCONFIRMED_DATA_UP << 5, 0);
        }
        if (!_isDefined(p.FOpts)) {
            p.FOpts = new Buffer('', 'hex');
        }
        if (!_isDefined(p.FPort)) {
            p.FPort = new Buffer('01', 'hex');
        }
        if (!_isDefined(p.FCnt)) {
            p.FCnt = new Buffer('0100', 'hex');
        }
        if (!_isDefined(p.FCtrl)) {
