...
function LoraPacket(packetContents) {
    ...
    this.toJSON = function() {
        ...
        if (that.isJoinRequestMessage()) {
            json.type = "join_request";

            json.PHYPayload = {}
            json.PHYPayload.full = asHexString(p.PHYPayload);
            json.PHYPayload.MHDR = asHexString(p.MHDR);
            json.PHYPayload.MACPayload = asHexString(p.MACPayload);
            json.PHYPayload.MIC = asHexString(p.MIC);

            json.MACPayload = {};
            json.MACPayload.AppEUI = asHexString(p.AppEUI);
            json.MACPayload.DevEUI = asHexString(p.DevEUI);
            json.MACPayload.DevNonce = asHexString(p.DevNonce);
        } else if (that.isJoinAcceptMessage()) {
            json.type = "join_accept";
            ...
            json.MACPayload = {}
            json.MACPayload.AppNonce = asHexString(p.AppNonce);
            json.MACPayload.NetID = asHexString(p.NetID);
            json.MACPayload.DevAddr = asHexString(p.DevAddr);
            json.MACPayload.DLSettings = asHexString(p.DLSettings);
            json.MACPayload.RxDelay = asHexString(p.RxDelay);
            json.MACPayload.CFList = asHexString(p.CFList);

            json.DLSettings = {}
            json.DLSettings.RX1DRoffset = that.getDLSettingsRxOneDRoffset();
            json.DLSettings.RX2DataRate = that.getDLSettingsRxTwoDataRate();
            json.RxDelay = that.getRxDelayDel();

            if(p.CFList.length === 16) {
                json.CFList = {}
                json.CFList.FreqCh4 = (that.getCFListFreqChFour());
                json.CFList.FreqCh5 = (that.getCFListFreqChFive());
                json.CFList.FreqCh6 = (that.getCFListFreqChSix());
                json.CFList.FreqCh7 = (that.getCFListFreqChSeven());
                json.CFList.FreqCh8 = (that.getCFListFreqChEight());
            }
        } else if (that.isDataMessage()) {
            json.type = "data";
            ...
            json.MACPayload = {}
            json.MACPayload.FHDR = asHexString(p.FHDR);
            json.MACPayload.FPort = (that.getFPort());
            json.MACPayload.FRMPayload = asHexString(p.FRMPayload);

            json.FHDR = {}

            json.FHDR.DevAddr = asHexString(p.DevAddr);
            json.FHDR.FCtrl = asHexString(p.FCtrl);
            json.FHDR.FCnt = asHexString(p.FCnt);
            json.FHDR.FOpts = asHexString(p.FOpts);


            json.direction = that.getDir();
            json.FCnt = that.getFCnt();
            json.FCtrlACK = that.getFCtrlACK();
            json.FCtrlADR = that.getFCtrlADR();
        }
        return json;
    };
